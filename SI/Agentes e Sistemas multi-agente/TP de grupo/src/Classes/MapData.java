...
public class MapData {
    ...
    public boolean enemiesAround(Position p, int enemyteam) {
        ...
        if (p.getY() == 0 || (p.getY()-1 > 0 && map[p.getX()][p.getY()-1].getAvailable() == enemyteam)) {
            enemiesCounter++;
        }

        // Baixo: há bordas ou inimigo
        if (p.getY() == map.length-1 || (p.getY()+1 < map.length && map[p.getX()][p.getY()+1].getAvailable() == enemyteam)) {
            enemiesCounter++;
        }

        // Esquerda: há bordas ou inimigo
        if (p.getX() == 0 || (p.getX()-1 >= 0 && map[p.getX()-1][p.getY()].getAvailable() == enemyteam)){
            enemiesCounter++;
        }

        // Direita: há bordas ou inimigo
        if (p.getX() == map.length-1 || (p.getX()+1 < map.length && map[p.getX()+1][p.getY()].getAvailable() == enemyteam)){
            enemiesCounter++;
        }


        
        return enemiesCounter == 4;
    }
    ...
    // Fazer jogada
    public Position registerMove(String team, Position atual, int direction) throws InvalidMove {
        Position dest = null;
        // Garantir que jogador não sai do mapa
        if(atual.getX() <= 14 && atual.getX()>= 0 && atual.getY()<= 14 && atual.getY() >= 0) {
            // Obter nova posicao
            switch(direction) {
                case 0: // up
                    if(atual.getY() > 0)
            ...
            if (dest != null && map[dest.getX()][dest.getY()].getAvailable() == 0) {
                map[atual.getX()][atual.getY()].setAvailable(0);
                int t = 2;
                if (team.equals("B"))
                    t = 1;
                map[dest.getX()][dest.getY()].setAvailable(t);
                map[dest.getX()][dest.getY()].setIdPlayerPos(map[atual.getX()][atual.getY()].getIdPlayerPos());
        ...
        if (dest == null) throw new InvalidMove();
        else
            return dest;
